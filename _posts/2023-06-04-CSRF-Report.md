...
## Mitigating CSRF Attacks {#section-4}
What we suggest.  
Zero Trust: Educate Employers about the dangers of clicking unknown links in emails. Check if the email is sent from a public email domain like `gmail.com`. Check if the domain name is misspelt. Utilize `virus total` a free url checking website to check if the url has been reported as malicious.
Use CSRF tokens.     
